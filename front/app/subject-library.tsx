...
function ResourceCard({
  ...
  const ogImage = item.openGraph?.ogImage;
...
export default function SubjectLibraryScreen() {
  ...
  const [form, setForm] = useState({ title: '', description: '', url: '', type: 'LINK' as ResourceType, tags: '' });
  ...
  const loadResources = useCallback(
    ...
    [subjectId, page, buildParams],
  );

  useFocusEffect(
    useCallback(() => {
      loadResources(true);
    }, [subjectId, activeType, sortBy]),
  );
            ...
            <TouchableWithoutFeedback>
              <View style={styles.modal}>
                ...
                <ScrollView horizontal showsHorizontalScrollIndicator={false} style={{ marginBottom: 12 }}>
                  {(['LINK', 'PDF', 'VIDEO', 'DOCUMENTO', 'OTRO'] as ResourceType[]).map((t) => (
                    <Pressable
                      key={t}
...
const styles = StyleSheet.create({
  ...
  filtersRow: { paddingHorizontal: 14, paddingBottom: 10, gap: 6 },
  chip: {
  ...
  chipDivider: { width: 1, backgroundColor: '#e2e8f0', marginHorizontal: 4 },
  ...
  tag: { backgroundColor: '#f8fafc', borderWidth: 1, borderColor: '#e2e8f0', borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2 },
